...
function ButtonUnArchive ({id, isArchive}) {
    const navigate = useNavigate();
    if(isArchive){
        return (
            <button className="action" type="button" title="Batal Arsip" 
                    onClick={() => {unarchiveNote(id); navigate('/arsip');}}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="currentColor" className="bi bi-bookmark-x" viewBox="0 0 16 16">
